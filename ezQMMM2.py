...
Reference:
    ...
    RCD/RC:  Lin & Truhlar, J. Phys. Chem. A 109, 3991-4004 (2005)
"""
...
class QMMMGenerator:
    ...
    def _get_element_from_mass(self, mass: float) -> str:
        ...
        for (lo, hi), elem in self.MASS_TO_ELEMENT.items():
            if lo <= mass <= hi:
                return elem
        # Fallback chain for common elements outside the table ranges
    ...
    def _tile_images(self, charges: list, qm_pos: np.ndarray,
        ...
        relative to the QM centroid — the (0,0,0) shell skip then correctly
            ...
            for iy in range(-ny, ny + 1):
                for iz in range(-nz, nz + 1):
                    if ix == 0 and iy == 0 and iz == 0:
                        continue                    # primary cell already included
    ...
    def extract_point_charges(self, qm_selection: str, cutoff: float,
        ...
        # Whole-residue inclusion: any atom within cutoff pulls in its residue
        residues_in = set()
        ...
        # Apply boundary scheme to primary-cell charges
        ...
        # distance_array(box=dimensions) finds atoms via PBC but keeps raw coords,
        # which may be a full box length away from QM. Remapping:
        #  (a) ensures written coordinates are near QM, and
        #  (b) makes the (0,0,0) skip in _tile_images match the primary set exactly.
        ...
        # Apply switching to all charges; box=None since positions are explicit
        switch_recs = []
    ...
    def _apply_switching_to_charges(self, charges, qm_pos, sw, cut, box,
        ...
        Distance r is the minimum distance from the charge to any QM atom.
        box=None is expected when image charges are present (explicit positions).
        n_primary marks the boundary between primary and image charges for
        SwitchRecord.is_image.
        ...
        # One vectorised distance call: (n_qm, N) -> min over QM axis -> (N,)
        all_dists = distances.distance_array(
            qm_pos, positions, box=box
        ).min(axis=0)

        # Vectorised quintic switching
        sw_range = cut - sw
    ...
    def _apply_boundary_scheme(self, mm_atoms, boundary_bonds, scheme):
        ...
        Modifies charges near the QM/MM covalent boundary.
        ...
        for qm_idx, mm1_idx in boundary_bonds:
            ...
            if scheme == 'Z1':
                ...
                # Zero MM1 and all MM2 atoms (first two consecutive MM atoms)
                removed_atoms.add(mm1_idx)
                charge_mods.append({
                    'type': 'removed', 'atom': mm1_atom,
                    'old_charge': mm1_charge, 'new_charge': 0.0,
                ...
                # Zero MM1, MM2, and MM3 (three consecutive MM atoms along the bond chain)
                removed_atoms.add(mm1_idx)
                charge_mods.append({
                    'type': 'removed', 'atom': mm1_atom,
                    'old_charge': mm1_charge, 'new_charge': 0.0,
    ...
    def _write_structure(self, frame: int, qm_atoms, mm_ag, base: Path,
        ...
        positions relative to the QM centroid, matching the point charge
        coordinates written to the QM/MM input file.

        Beta (tempfactor) column:
          9 = QM atom
        ...
        # Temporarily remap MM positions to minimum image for consistent
        # coordinates between PDB and input files
        orig_mm_pos     = mm_ag.positions.copy()
        mm_ag.positions = self._remap_positions_array(orig_mm_pos, qm_center, box)

        # Vectorised beta assignment; QM last so it overwrites MM on any overlap
    ...
    def _write_topology(self, psf_source: str, output_dir: Path,
                        prefix: str) -> Path:
        """
        Copy the original PSF once to the output directory as the shared
        topology file for all per-frame PDB files.
        PSF content is frame-independent (no coordinates, no box).
        """
    ...
    def _write_orca(self, fname, coords, charges, method, basis,
                    charge, mult, keywords, custom_blocks):
        """Write ORCA input file with optional external point charge file."""
        with open(fname, 'w') as f:
            ...
            if charges:
                # Point charges go in a separate .pc file; ORCA reads it via
                # %pointcharges. Path is the bare filename so ORCA finds it
                # when run from the same directory as the .inp file.
    ...
    def _write_qchem(self, fname, coords, charges, method, basis,
                     charge, mult, keywords, custom_blocks):
        """Write Q-Chem input file with inline $external_charges block."""
        with open(fname, 'w') as f:
            f.write("$molecule\n")
            ...
            if charges:
                f.write("\n$external_charges\n")
                # Q-Chem format: x  y  z  q  (Angstrom)
                for q, x, y, z in charges:
    ...
    def _write_psi4(self, fname, coords, charges, method, basis,
        ...
        QM coordinates: Angstrom (Psi4 molecule block default).
        Point charges: converted to Bohr (Psi4 extern requires Bohr).
        """
...
class QMMMGenerator:
    ...
    def _write_structure(self, frame: int, qm_atoms, mm_ag, base: Path,
        ...
        remapping applied to the point charges in the .in file.
        ...
        In VMD: load the shared *_struct.psf first, then load each
        *_frame<N>_struct.pdb as a new frame. Color by Beta to visualise
        QM/MM partitioning.
        """
        qm_idx = set(qm_atoms.indices)
        mm_idx = set(mm_ag.indices)
    ...
    def _write_topology(self, psf_source: str, output_dir: Path, prefix: str):
        """
        Copy the original PSF once to the output directory as a shared
        topology file for all per-frame PDB files.
        """
        dest = output_dir / f"{prefix}_struct.psf"
        shutil.copy(psf_source, dest)
